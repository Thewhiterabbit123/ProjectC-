...
Car::Car(b2World* world) {
    ...
    jointDef.localAnchorA.Set(-CAR_WIDTH/2/RATIO-TIRE_WIDTH/2/RATIO-INTERVAL/RATIO, 
                               CAR_HEIGHT/4/RATIO+INTERVAL/RATIO );
    ...
    tire->setCharacteristics(maxForwardSpeed, maxBackwardSpeed, 
                             backTireMaxDriveForce, backTireMaxLateralImpulse);
    jointDef.bodyB = tire->getBody();
    jointDef.localAnchorA.Set(CAR_WIDTH/2/RATIO-TIRE_WIDTH/2/RATIO+(INTERVAL+6)/RATIO, 
                              CAR_HEIGHT/4/RATIO+INTERVAL/RATIO );
    world->CreateJoint( &jointDef );
    m_tires.push_back(tire);
